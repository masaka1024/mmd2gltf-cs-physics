...
static class TsBase
{
    ...
    static int Main()
    {
        ...
        // 1) Sub1 ビット一致 (リセット無し)
        var s1 = Run(1);
        // ★2026-08-23 更新: 完全セットv1 の既定化にともなう更新。
        //    旧 10.995/22.064/112.203 (2026-08-21〜) -> 新 11.233/23.144/111.191。
        //    ★このハーネスは env を一切読まない (フラグの A/B ができない)。
        //      したがって「切替前後で不変」と読めた過去の観測は、
        //      **フラグが当たっていなかっただけ**である。出荷既定だけを測る器と理解すること。
        //    さらに前: 11.041/22.498/111.822 は 2026-08-13 の慣性修正で失効していた。
        bool bitok = Math.Abs(s1.calmMed - 11.233f) < 5e-4f && Math.Abs(s1.calmP90 - 23.144f) < 5e-4f && Math.Abs(s1.calmMax - 111.191f) < 5e-4f;
        L($"\n[Sub1(リセット無) ビット一致確認] 平時 中央={s1.calmMed:F3} p90={s1.calmP90:F3} max={s1.calmMax:F3}  => {(bitok ? "★ベースラインと一致 (11.233/23.144/111.191)" : "不一致(!)")}");
