...
//
//  なぜ要るか (タスク24 の続き):
//    神託で「モデルB は 髪は止まる / スカートは振動」と分かった。スカートは参照でも
//    振動しているので、当エンジンの「中央値悪化 197→284」が過剰なのか妥当なのかは
//    **振幅を参照と同じ物差しで比べないと決まらない**。
//    参照側は PMXエディタのベイク VMD から `analyze_static_bake.py` で
...
static class BoneDp
{
    ...
    public static int Run(PmxPhysicsModel model, string filter)
    {
        ...
        L("    この2つを並べれば、既定と3点構成のどちらが参照に近いかが決まる。");
        L("    参考: 参照データ(モデルA 髪)の静区間フロア = 0.0028 〜 0.0128");
        L("=".PadRight(104, '='));
