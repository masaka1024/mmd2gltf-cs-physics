...
- **長い鎖(節数が多い揺れ物)はソルバ反復が届かず、親との距離が伸び縮みする**。
  Gauss-Seidel は 1 反復あたりジョイント 1 本ぶんしか情報を伝えないので、節数が反復数
  (`SolverIterations`=10) を超える鎖では根元の拘束が末端まで届きません。
  実測 (モデルM の しっぽ = 13 節、ダンス 233 秒、実効刻み 1/120): 鎖の最大比 3.16 倍
  (主に腰ベルト)。窓ごとの推移は単調でないので、誤差が溜まるラチェットではなく
  一過性のたわみです。実効刻みを 1/30 まで粗くすると しっぽ３ が 5.19 倍まで開きます。
  緩和策は `PhysicsWorld.JointVelocityIterations` (既定 0 = 従来と完全に同じ)。
  代償があるので既定は変えていません → 「ソルバ反復とすり抜けのトレードオフ」節。
...
`JointVelocityIterations` は「速度求解でジョイントだけを追加で回す回数」です
(接触は `SolverIterations` のまま。接触の反復を増やすと待機区間の揺れが悪化するため)。
UE5 移植版で 13 節のしっぽが伸び続ける破綻の対策として入り、こちらへ逆輸入しました。

鎖のたわみは確かに消えますが、**髪が体へ沈む量が増えます**。ジョイントが最後に
30 回多く解かれるぶん、接触の力積を押し戻すためです。実測:

| `JointVelocityIterations` | 鎖の最大比 (モデルM 233秒) | 深貫入>0.5 のイベント数 (モデルA 7001f) | 静区間 角度差 中央 |
|---:|---:|---:|---:|
| 0 (既定・従来) | 3.16 | **0** | 19.84° |
| 12 | 1.26 | 15 | 19.48° |
| 15 | 1.19 | 102 | 20.76° |
| 20 | 1.12 | 277 | 19.35° |
| 40 | 1.07 | 643 | 21.34° |

MMD 自身の深貫入(幾何)は同じ分母で 66 件なので、12 まではMMDより少なく、20 以上は
MMDを超えます。`bonecheck` の 12窓比 中央は 0.9867 → 1.0033 (40 のとき) とむしろ 1.0 に
近づくため、**忠実度の指標は改善・衝突は悪化**という向きです。

`JointMaxCorrectionVel` (位置補正速度の上限) は UE5 側では必須でしたが、こちらでは
効果がありません (40+30 で 1.07 → 1.09)。あちらは誤差が単調に溜まって補正が上限へ
張り付く状態でしたが、当エンジンはその状態に入らないためです。

**既定は 0 (従来と完全に同じ) のまま**です。1モデルの実測で既定を動かさない方針
(過去に1モデルで決めた推奨値が追認で覆った実例あり)。長い鎖を持つモデルで実害が出たときに、
上表を見ながら 12 前後から上げてください。
...
| `perf` | 位相別プロファイル | 不要 |
| `chainstab` | 揺れ物の鎖が伸び縮みしないか（親との距離をバインド長との比で見る） | **要** |

> ⚠ `testdata/modelA.pmx` はリポジトリに含まれません（再配布回避）。無い状態では
> `restsim` / `bonecheck` / `hairfid` / `chainstab` は `[SKIP]` になります。
> 手元のモデルを `MMD_TEST_PMX` で指せば忠実度の数値回帰は回せます（「必要な外部データ」参照）。
