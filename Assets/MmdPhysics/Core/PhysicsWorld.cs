...
namespace BulletPhysics
{
    ...
    public sealed class PhysicsWorld
    {
        ...
        /// <summary>減衰係数の上限。★2026-08-30 タスク142 (freeze 明け) に **0.999 → 1.0** へ。
        /// Bullet 2.75 の受け口は `[0,1]` (btRigidBody.cpp:139) なので **1.0 が Bullet 準拠**。
        /// ★2026-08-27 タスク84 の「1.0 のほうが良い」撤回は、参照データが無効だったため。
        ///   健全な参照 (モデルZ 実物レプリカ) が用意できたので、根拠つきで動かせるようになった。
        /// 根拠 (タスク136/138/140/141):
        ...
        ///   ・手元 43 モデル中 **22 (51%)** で発火、全減衰値の **31%** が該当 (モデルZ 特有ではない)。
        ///   ・A/B 前哨: 悪化ゼロ・`d<=0.999` のみのモデルはビット同一・発火帯は揺れがわずかに締まる。
        /// ★**撤廃ではなく上限 1.0** である理由: `d>1.0` の剛体 (モデルZ に 178 件・最大 1.2) を
        ///   素通しすると `(1-1.2)^dt` が **NaN** になる。上限 1.0 は Bullet の受け口と一致し NaN も防ぐ。
        /// ★従来値へは env `DAMPCLAMP=0.999` で戻せる。</summary>
        public static float DampingClampMax = 1.0f;
